...
def main():
    ...
    for i, line in enumerate(lines):
        if "{/* THE LINEAR STORY LAYOUT */}" in line:
            start_idx = i
        if "Edge cases are the product." in line:
            # For linear layout, it looks like:
            #               <h4 style={{ ... }}>
            #                 Edge cases are the product.
            #               </h4>
            #               <p style={{ ... }}>
            #                 In B2B operations, failure states...
            #               </p>
            #             </motion.div>
            #           </div>
            # So the end is about 6 lines after the h4.
            pass
        if "In B2B operations, failure states (like a stuck order) are core UX problems." in line:
            # the text is on this line. The closing p, motion.div, div are next.
            end_idx = i + 3
            break
...
"""
        ...
        with open(filepath, "w") as f:
            f.writelines(lines)
        print("Reverted to Bento Layout successfully.")
    else:
